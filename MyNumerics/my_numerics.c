...
void hist_add(Hist* H, double x){
    ...
    //checkt obere Grenze von jedem bin in wachsender Reihenfolge
    //sobald x <= obere Grenze von bin ist der bin gefunden
    /*if(x <= H->high && x >= H->low){
        //Breite von bins, ang dass alle gleich sind
    ...
    //Wenn x = obere grenze ist, dann geht die Formel nicht auf
    //dann x manuell in letzten bin eintragen+
    if(x <= H->high && x >= H->low){
        if(x != H->high){
