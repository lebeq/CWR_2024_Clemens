            ...
            for(int j = 0; j<N; j++){
                switch(F[i][j].type){
                    case INSIDE:
                        ...
                        F[i][j].val = (F[i-1][j].val + F_old[i+1][j].val + F[i][j-1].val + F_old[i][j+1].val)/4.0;
                        break;
                    case DIRICHLET:
                        break;
                    default:
...
int main(){
    int N = 128; //Anzalh gitterpunkte in eine Richtung -> N^2 Gitterpunkte insg.
    ...
    PoissonGaussSeidel(N,F,max_iter,err_tol);
